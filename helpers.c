...
int request_confirmation(GtkWidget *window, char *question, char *website, char *username);
...
int request_confirmation(GtkWidget *window, char *question, char *website, char *username)
{
    ...
    gtk_window_set_title(GTK_WINDOW(dialogue), "Confirmation");
    ...
    // buttons
    gtk_dialog_add_button(GTK_DIALOG(dialogue), "No", GTK_RESPONSE_REJECT);
    gtk_dialog_add_button(GTK_DIALOG(dialogue), "Yes", GTK_RESPONSE_ACCEPT);
